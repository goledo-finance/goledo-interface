    ...
    return (
      <div className="relative max-w[400px] min-w-[300px] bg-white rounded  overflow-hidden">
        <div className={type === 'info' ? 'p-[24px]' : 'pl-[56px] py-[24px] pr-[24px]'}>
          ...
          <p className="mb-[8px] leading-[22px] text-[16px] text-[#3D3F4C] font-semibold max-w-[280px]">
          ...
          {(typeof content === 'string' || content.text) && (
            <p className="leading-[18px] text-[14px] text-[#898D9A] max-w-[320px]">{typeof content === 'string' ? content : content.text}</p>
          )}
        ...
        {duration ? <a.div className="absolute bottom-0 w-full h-[4px] bg-gradient-to-l from-[#15C184] to-[#808BE7]" style={props} /> : null}
      </div>
    );
  }
);
