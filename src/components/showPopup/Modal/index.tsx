...
const TransactionSubmittedContent: React.FC<{ TxnHash: string; action: string; blockExplorerUrl?: string; duration?: number }> = memo(
  ({ TxnHash, action = 'Transaction', blockExplorerUrl, duration }) => {
    ...
    return (
      <div className="relative w-[340px] min-h-[192px] p-[24px] text-center bg-gray-200 rounded-lg overflow-hidden">
        ...
        {blockExplorerUrl && (
          <a
            className="block text-[14px] leading-[18px] text-[#808BE7] text-left break-words hover:underline"
            href={`${blockExplorerUrl}/transaction/${TxnHash}`}
            target="_blank"
            rel="noopener"
          >
            {TxnHash}
        ...
        {duration ? <a.div className="absolute left-0 bottom-0 w-full h-[4px] bg-gradient-to-l from-[#15C184] to-[#808BE7]" style={props} /> : null}
      </div>
    );
  }
);
