...
import { type Props as PopperProps } from '../Popper';
...
const BalanceText: React.FC<Props> = ({ className, balance, status, id, symbol = '', decimals, placement }) => {
  if (!balance) {
    return (
  ...
  return (
    <Tooltip text={`${numFormat(decimalStandardUnit)} ${symbol}`} placement={placement} disabled={!needAabbreviate || noughtLen < 6} interactive delay={500}>
      <span className={className} id={id}>
