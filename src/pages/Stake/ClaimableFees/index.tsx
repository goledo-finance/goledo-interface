...
const ClaimableFees: React.FC = () => {
  ...
  const data = claimableFees?.slice(1)?.map((item) => ({ ...item, price: item.symbol === 'GOL' ? goledoUsdPrice?.mul(item.balance)! : item.price }));
  const totalClaimablePrice = data?.reduce((acc, item) => acc.add(item.price ?? Zero), Zero);

  if (!totalClaimablePrice || totalClaimablePrice.equalsWith(Zero)) return null;
  return (
    <Card title="Claimable Fees in v1 Contract" id="stake-claimable-fees-card">
