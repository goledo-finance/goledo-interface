...
export const createCFXData = (account: string) => WETHGatewayContract.interface.encodeFunctionData('borrowETH', [import.meta.env.VITE_LendingPoolAddress, account, '0'])
...
export const handleBorrow = async ({ amount, symbol, tokenAddress }: { amount: Unit; symbol:string; tokenAddress: string;}) => {
    ...
    try {
        let data: string;
        if (symbol === 'CFX') {
            data = WETHGatewayContract.interface.encodeFunctionData('borrowETH', [import.meta.env.VITE_LendingPoolAddress, account, '0']);
        } else {
